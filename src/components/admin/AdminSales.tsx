...
function AdminSalesContent() {
  ...
  const fetchSubscriptionsAndPrices = useCallback(async () => {
    try {
      ...
      console.error('[AdminSales] Fetch error:', err);
      if (isMountedRef.current) {
  ...
  useEffect(() => {
    console.debug('[AdminSales] Setting up Realtime channel');
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          console.debug('[AdminSales] Realtime event:', payload.eventType, payload.new);
      ...
      .subscribe((status) => {
        console.debug('[AdminSales] Realtime status:', status);
      });

    return () => {
      console.debug('[AdminSales] Cleaning up Realtime channel');
  ...
  if (loading) return <div className="text-muted-foreground text-sm">Cargando métricas de ventas...</div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <button
            onClick={() => fetchSubscriptionsAndPrices()}
...
export function AdminSales() {
  return (
    <AdminSectionErrorBoundary sectionName="Ventas">
      <AdminSalesContent />
